...
// Apparently, tertiary (triple star) systems exist, but, usually as a binary plus a distant third. Otherwise, would be rather chaotic as we observe here!
val (pos, size, initSpeed, normalize, sample) = (50, 5, 1.0, 100, 4)
val brushSize = 0.2 // paints the orbits. Set to 0 to turn off
...
animate {
    ...
    if (brushSize > 0 && step % sample == 1) { for ( (p, c) <- List((p1, c1), (p2, c2), (p3, c3)) ) draw(trans(p.x, p.y) * c -> Picture.circle(brushSize))}; step += 1
    for (p <- List(p2, p3)) {
        val (fx, fy) = dv(p1, p)
        dx1 += fx; dy1 += fy
    }
