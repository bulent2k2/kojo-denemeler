...
silVeSakla()
tanım dürt = { dez r = 0; rastgele(10); yaz(s"$r "); r } // başlangıç yerlerini biraz dürtelim ki farklı yörüngeler ortaya çıksın
dez cc1 = götür(yer + dürt, yer + dürt) -> c1; cc1.çiz
dez cc2 = götür(-yer + dürt, -yer + dürt) -> c2; cc2.çiz
dez cc3 = götür(-yer + dürt, yer + dürt) -> c3; cc3.çiz
...
canlandır {
    ...
    adım += 1
    eğer (fırçaBoyu > 0 && adım % örnekleme == 1) için ((p, c) <- Dizi((p1, r1), (p2, r2), (p3, r3))) { dez y = götür(p.x, p.y) * c -> Resim.daire(fırçaBoyu); y.çiz }
    için (p <- Dizi(p2, p3)) {
        dez (fx, fy) = dhız(p1, p)
        dx1 += fx; dy1 += fy
    }
