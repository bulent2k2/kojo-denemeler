// ~/src/kojo/git/kojo/src/main/resources/samples/tr/angles.kojo

// Açı nedir herkes bilir ama nasıl ölçülür görmek ister misin?
// 360 derece ya da 2 çarpı pi radyan deriz tam bir daireye. 
// Dereceleri anladık dik açı yani 90 derece çarpı 4 tam bir dönüş.
// Bu devinimli çizimle radyan kavramını anlayalım.
// Radyan İngilizcede radian diye yazılır. Radius yani yarıçaptan gelir.
...
def açıÇiz(start: Kesir, açı: Kesir) = rot(start) -> GPics(
    ...
    Picture.arc(yçBoyu / 4, açı)
)

// tuvale 7 biçim çizim yapacağız. 
var birYay = yayÇiz(0)
var birYarıçap = yarıçapıÇiz(0)
var yçYazısı = trans(yçBoyu / 2, -5) -> Picture.text("yç", 20)
