// if we randomly select three points on a circle,
...
def drawIt(ps: Points, fill: Boolean = false) = {
    ...
    draw(penColor(blue) * trans(0, 0) -> Picture.circle(r)) // circle
    draw(fillColor(fc) * trans(0, 0) -> Picture.circle(4)) // center is filled if it is inside the triangle
    ps.map(p => draw(fillColor(fc) * trans(p._1, p._2) -> Picture.circle(4))) // the corners of the triangle
    ps.zip(ps.last :: ps).map {
      case (p1, p2) => { // the edges of the triangle
        val (x1, y1, x2, y2) = (p1._1, p1._2, p2._1, p2._2)
        draw(trans(x1, y1) -> Picture.line(x2-x1, y2-y1))
      }
    }
}
