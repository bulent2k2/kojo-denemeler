...
durum sınıf Cisim(kütle: Kesir, ilkKonum: Nokta, hız: Yöney, res: Res) {
    ...
    dez r = götür(ilkKonum.x, ilkKonum.y) * res.rengi -> Resim.daire(res.çap)
    r.çiz
}
...
silVeSakla; yaklaşXY(0.3, 0.3, konum, -konum)
...
yap { // canlandır {
    zamanTut("Bin adım atmak")(yinele(1000) {
        eğer (fırçaBoyu > 0 && adım % örnekleme == 0) 
            için (c <- cisimler) { 
        ...
        için (c <- cisimler) {
            için (öbürü <- cisimler eğer öbürü != c)
                c.hız.ekle(dHız(c.konum, öbürü.konum, öbürü.kütle))
            c.kaydır
        }
    })("tuttu")
    satıroku("Devam etmek için return tuşuna basın.")
