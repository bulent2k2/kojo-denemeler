...
var step = 0; // count how many times the loop below ran so far
animate {
  if (step % sampleEveryNsteps == 0) for (b <- bodies) draw(trans(b.p.position.x, b.p.position.y) * b.pic.ct -> Picture.circle(2)); step += 1
  for (b <- bodies) {
