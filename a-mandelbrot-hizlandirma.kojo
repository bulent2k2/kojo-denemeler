/*
 ...
 tekrar ileri git. İlk yaklaşma bir iki saniye alır. Ama ileri/geri okları
...
/*
 ...
 x'e varsıl sayının gerçek (real) parçası da denir.
 y'ye de hayal (imaginary) parçası denir.
...
case class VarsılSayı(x: Kesir, y: Kesir) {
    ...
    def uKare = x * x + y * y // uzunluk < 2 yerine daha hızlı olsun diye uKare < 4 şeklinde kullan
    def kare = { // v *= v yerine daha hızlı olsun diye bunu kullan
        val xy = x * y
        val (xPlusY, xMinusY) = (x + y, x - y)
        // VarsılSayı(x * x - y * y, xy + xy)
        VarsılSayı(xPlusY * xMinusY, xy + xy)
    }
}
...
// Epey faydalı olacak:
object Dörtgen {
    def apply(solalt: Nokta, sağüst: Nokta): Dörtgen = {
        new Dörtgen(solalt.x, sağüst.x, solalt.y, sağüst.y)
    }
}
...
/*
 ...
 demekte fayda var. MK'yi aşağıdaki m(v) işleviyle tanımlarız:
     ...
     m(1) = v
     m(2) = v^2 + v                 (v^2 = v*v)
     m(3) = v^4 + 2*v^3 + v^2 + v   (v^3 = v*v*v  ve v^ = v*v^3)
     ...
...
def mKümesi(d: Dörtgen): İmge = {
    ...
    if (bellek.eşli(d)) bellek(d) else {
        ...
        val oran = ((d.x2 - d.x1) / kenar, (d.y2 - d.y1) / kenar)
        val iri: Kesir = (1.0 * yinelemeSınırı) * kenar * kenar
        zamanTut(f"mKümesini bulmak ($iri%2.2e kere yineleme)") {
            kümeElemanlarınıBul(img, d, oran)
        }()
        bellek eşle (d -> img)
        img
    }
...
def kümeElemanlarınıBul(img: Bellekteİmge, d: Dörtgen, oran: (Kesir, Kesir)) = {
    def mKümeYineleme(xi: Sayı, yi: Sayı) = {
        val x = d.x1 + xi * oran._1
        val y = d.y1 + yi * oran._2
        val v = VarsılSayı(kayma + x, y)
        var z = VarsılSayı(0, 0)
        var i = 0
        while (z.uKare <= 4 && i < yinelemeSınırı) {
            // z *= z; z += v
            z = z.kare + v // işte bütün küme buradan çıkıyor!
            i += 1
        }
        (i, z.uKare <= 4, x, y)
    }
    def mKümeYinelemeBazenBirazDahaHızlı(xi: Sayı, yi: Sayı) = {
        val x = d.x1 + xi * oran._1
        val y = d.y1 + yi * oran._2
        val v = VarsılSayı(kayma + x, y)
        val z = VarsılSayı(0, 0)
        var i = 0
        // daha hızlı olsun diye z yerine zr (real) ve zi (imaginary),
        // v yerine vx ve vy
        var (zr, zi) = (0.0, 0.0)
        var (zrs, zis) = (zr * zr, zi * zi)
        val (vx: Kesir, vy: Kesir) = (v.x, v.y)
        while (zrs + zis <= 4 && i < yinelemeSınırı) {
            zi = zr * zi
            zi += zi
            zi += vy
            zr = zrs - zis + vx
    ...
    val f1 = mKümeYineleme _
    val f2 = mKümeYinelemeBazenBirazDahaHızlı _  // f2 == f1
    for { xi <- 0 until kenar; yi <- 0 until kenar } {
        val (i, elemanMı, x, y) = f1(xi, yi)
        // küme içindeki noktalar hep siyah diğerleri de renkli olacak:
        imgeNoktasınıKur(img, xi, yi, if ( /*z.uzunluğu <= 2*/ elemanMı) siyah else renk(i, x, y))
    }
}
